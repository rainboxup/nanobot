...
- workspace 已安装技能（`GET /api/skills*` 的稳态视图）：`source=workspace`，`origin_source=workspace`，`install_source=local`
- 刚通过 `POST /api/skills/install` 安装本地托管条目后的响应：`source=workspace`，`origin_source=store`，`install_source=local`
