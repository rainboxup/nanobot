...
class LiteLLMProvider(LLMProvider):
    ...
    def __init__(
        ...
        # NOTE: We intentionally avoid mutating global environment variables here.
        # Multi-tenant gateways may route requests for different users concurrently.
        # Instead, we pass api_key/api_base per-request to LiteLLM.
    ...
    async def chat(
        ...
        # Pass api_key/api_base per request (supports multi-tenant usage).
        if self.api_key:
            kwargs["api_key"] = self.api_key

        # Use explicit api_base when configured (gateways/local endpoints).
        if self.api_base:
            kwargs["api_base"] = self.api_base
        else:
            # Some providers require an explicit base URL even in standard mode.
            # Prefer registry defaults instead of global env vars.
            spec = find_by_model(model)
