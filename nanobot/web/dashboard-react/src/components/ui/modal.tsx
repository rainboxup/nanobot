...
export function Modal({ isOpen, onClose, title, description, children, footer }: ModalProps) {
  ...
  React.useEffect(() => {
    ...
    // Focus the close button by default to keep keyboard navigation predictable.
  ...
  return (
    ...
    >
      <div
        ...
        className="relative w-full max-w-lg rounded-xl border bg-background p-6 shadow-lg animate-in zoom-in-95 slide-in-from-bottom-2"
      >
        <button
          onClick={onClose}
          type="button"
        ...
        {footer && <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end sm:space-x-2 mt-4 w-full">{footer}</div>}
      </div>
    </div>
  )
}
