...
sys.modules.setdefault(
    "slackify_markdown",
    types.SimpleNamespace(slackify_markdown=lambda text: f"{text}\n" if text else ""),
)


def _make_channel(config: SlackConfig):
...
setattr(_FakeAsyncWebClient, "chat_postMessage", _FakeAsyncWebClient.chat_post_message)


@pytest.mark.asyncio
async def test_send_uses_thread_for_channel_messages() -> None:
    channel = _make_channel(SlackConfig(enabled=True))
...
async def test_send_updates_reaction_when_final_response_sent() -> None:
    channel = _make_channel(SlackConfig(enabled=True, react_emoji="eyes"))
    fake_web = _FakeAsyncWebClient()
    channel._web_client = fake_web
...
@pytest.mark.asyncio
async def test_send_does_not_update_reaction_for_progress_messages() -> None:
    channel = _make_channel(SlackConfig(enabled=True, react_emoji="eyes"))
    fake_web = _FakeAsyncWebClient()
    channel._web_client = fake_web
